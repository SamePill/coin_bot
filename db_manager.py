...
def cleanup_old_trade_logs(retention_days=90):
    """💡 DB 최적화: 설정된 기간이 지난 매매 로그(trade_logs)를 내 계정에서 자동 삭제"""
    conn = None
    try:
        conn = pool.connection()
        with conn.cursor() as cur:
            # 삭제 대상 개수 확인
            cur.execute("SELECT COUNT(*) FROM trade_logs WHERE account_id = %s AND trade_time < NOW() - INTERVAL %s DAY", (ACCOUNT_ID, retention_days))
            count = cur.fetchone()[0]
            if count > 0:
                cur.execute("DELETE FROM trade_logs WHERE account_id = %s AND trade_time < NOW() - INTERVAL %s DAY", (ACCOUNT_ID, retention_days))
                conn.commit()
                print(f"🧹 [DB 최적화] {ACCOUNT_ID} 계정의 {retention_days}일 경과한 매매 로그 {count}건을 삭제했습니다.")
    except Exception as e:
        print(f"❌ DB 매매 로그 최적화 오류: {e}")
    finally:
        if conn: conn.close()
